describe('Employee Wizard', function() {
...
before(function() {
	...
	beforeEach(function(){
		...
		Cypress.Cookies.preserveOnce('.AspNetCore.Antiforgery.IT8a6MuPYuY', '.AspNetCore.Session','.AspNetCore.Mvc.CookieTempDataProvider','new_username','FavouriteMenus')
		cy.wait(2000)
    })
	
	
	it('successfully loads', function() {
		cy.visit('http://192.168.0.77:8070/') 
	})
	...
	it('Change Company', function() {
		cy.get('[onclick="changeCompanyModal()"]').invoke('text').then((text) => {
			...
			if(comnpanyname!='cypressTest')
			 {
				cy.get('[onclick="changeCompanyModal()"]').click({force: true})
				cy.wait(1000)
				cy.get('[type="radio"]').check('10',{force: true})
	...
	it('Navigate Employee list and open EmpWizard', function() {
	   cy.visit('http://192.168.0.77:8070/Employee/Employee/EmployeeList')
	   cy.wait(2000)
	   
	   cy.xpath('//*[@id="mainRenderBody"]/div[2]/div/div/div[1]/nav/div/ul[2]/li[1]/a/label').invoke('text').then((text) => {
				  expect(text.trim()).equal('Employee Wizard')
					cy.log(text.trim())
		 })
	   
	   cy.get('[onclick="getEmployeeWizard();"]').eq(1).click({force: true})
	})
	...
	cy.wait(1000)
	cy.get('select[name=esilocation]').select('Mumbai',{force: true})
	...
	it('Employee wizard Employee Details', function() {
	   ...
	   cy.wait(2000)
		...
		cy.get('input[name=FATHERNAME]').type('subhash')
		 ...
		 cy.get('select[name=DEPARTMENT]').select('IT dept',{force: true})
	   ...
	   cy.get('input[name=PENSIONFUND]').then($input => {
		...
		cy.get('select[name=LOCATION]').select('MUMBAI',{force: true})
		...
		cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[2]/form/div[2]/div/button[2]').click({force: true})
		cy.get(".toast-message").invoke('text').then((text) => {
	...
	it('Employee wizard Personal Details', function() {
		...
		cy.get("#empWizardTitle").then(($span) => {
		   ...
		   cy.get('input[name=PERSONALEMAIL]').click({force: true})		
			...
			cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[3]/form/div[2]/div/button[2]').click({force: true})
			//cy.get('#btnPersonalDetailSave').contains('Next').click({force: true})
     ...
     it('Employee wizard Earning Details', function() {
		 cy.get("#empWizardTitle").then(($span) => {
             var employeedetailsheadertext = $span.text();
			   expect(employeedetailsheadertext).equal('Earning Details')
         })
		...
		//cy.get('input[name=FFIELD]').type('2000')
		
		//cy.get('input[name=FPROJ]').click({force: true})		
		//cy.get('input[name=FPROJ]').type('1000')
		
		//cy.get('input[name=FHRA]').click({force: true})		
		//cy.get('input[name=FHRA]').type('500')
		
		cy.get('input[name=TDS]').click({force: true})		
		cy.get('input[name=TDS]').type('200')
		cy.wait(2000)
		
		cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[4]/div[2]/button[2]').click({force: true})
		//cy.get('#btnEarningDeductionSaveNext').contains('Next').click({force: true})
     ...
     it('Employee wizard Assign Matrix', function() {	
		
		 cy.get("#empWizardTitle").then(($span) => {
             var employeedetailsheadertext = $span.text();
			   expect(employeedetailsheadertext).equal('Assign Matrix')
         })
		...
		cy.get('input[type="search"]').type('cy01')
		cy.wait(2000)
		cy.get('.select2-results__option--highlighted').click({force: true})
		  ...
		  cy.wait(1000)
		  cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[5]/form/div[4]/div/button[2]').click({force: true})
		  //cy.get('#btnApprovalMatrixSave').contains('Next').click({force: true})
			cy.get(".toast-message").invoke('text').then((dedutext) => {
				expect(dedutext.trim()).equal('Manager Assigned Successfully.')
				cy.log(dedutext.trim())
			})  
	     cy.get(".toast-message").click({force: true})
	 })
		
	 it('Employee wizard Shift Details', function() {
		...
		cy.get('#dateRange').click().then(input => {
		    input[0].dispatchEvent(new Event('input', { bubbles: true }))
			input.val('13/01/2020 to 15/01/2020')
	   })
		 cy.get('#shiftNameDrop').select('General',{force: true})
		 
		 //cy.get('#loadMachineNo').select('Biometrix',{force: true})
		 
		 cy.get('input[name=deviceNo]').click({force: true})
		 cy.get('input[name=deviceNo]').type('1')
		 
		 cy.wait(1000)
		  cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[6]/div/button[2]').click({force: true})
	...
	it('Employee wizard Leave Details', function() {
		 cy.get("#empWizardTitle").then(($span) => {
             var employeedetailsheadertext = $span.text();
			   expect(employeedetailsheadertext).equal('Leave Details')
         })
		cy.wait(1000)	
		 cy.get('#LeaveOpening').click({force: true})
		cy.get('#LeaveOpening').type('20')
		cy.wait(2000)
		cy.get('#chkCredit_PL').check({force: true})
		cy.wait(1000)
		cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[7]/form/div/div/div/button[2]').click({force: true})
	})	
		
	it('Employee wizard SelfService Details', function() {
		...
		cy.wait(2000)
		cy.xpath('/html/body/div[2]/div[2]/div[1]/div[2]/div[2]/div/div/div[3]/div[2]/div/div/div/div[2]/div/div/div/div/div[2]/div[8]/div/button[2]').click({force: true})
