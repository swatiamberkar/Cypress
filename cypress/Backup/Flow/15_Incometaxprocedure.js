...
describe('ITAX Procedure', function() {
	...
	it('Search Specific Emp code you want to incometax process', function() {
		...
		cy.wait(2000)
		cy.contains('li','Incometax test(CY7)').click({force: true})
		cy.wait(2000)
	})
...
it('Income Tax Declaration Entry(HRA Metro Rent Paid)', function() {
	...
	cy.get(".toast-message").invoke('text').then((text) => {
			...
			cy.get('#drpEntryMonth').select('July',{force: true})
			}
...
it('Income Tax Proof Entry with yes ', function() {
	...
	cy.xpath('//*[@id="modalActionButtonDiv"]/div/button').click({force: true})
	  cy.get(".toast-message").invoke('text').then((text) => {
		...
		.then(fileContent => {
		cy.get('[data-field="House_Rent"]').eq(0).upload({
		fileContent,
		fileName: filePath,
	...
	cy.xpath('//*[@id="modalActionButtonDiv"]/div/button').click({force: true})
	  cy.get(".toast-message").invoke('text').then((text) => {
		...
		.then(Cypress.Blob.binaryStringToBlob)
		.then(fileContent => {
		cy.get('[data-field="House_Rent"]').upload({
		fileContent,
		fileName: filePath,
		mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
		encoding: 'utf8'
