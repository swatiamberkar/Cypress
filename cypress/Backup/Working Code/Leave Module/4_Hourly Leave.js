describe('Attendence Process ', function() {
	...
	let hourlyLeave = 
    {LeaveType: "SpecialLeave" , EntryDate: "13/04/2020", InDate_HH: "9", InDate_MM: "00", OutDate_HH: "10", OutDate_MM: "00", Minute: "60"}
...
*/
	...
	*/
	...
	it('Leave Cannot be Applied,Payroll Completed for April Month',function(){
	const { softAssert, softExpect } = chai;
		cy.navigate_EmployeeLeave()
		...
		cy.get(".toast-message").invoke('text').then((text) => {		
			cy.log(text.trim())
			softExpect(text.trim()).to.eq('Leave Cannot be Applied,Payroll Completed for April Month');
		})	
